...
### Administrador_de_Banco_de_Dados
...
[https://sdk.collaboraonline.com/docs/installation/CODE_Docker_image.html](https://sdk.collaboraonline.com/docs/installation/CODE_Docker_image.html)

1. [Criar a rede](Producao.md#Rede_666)
...
Acesse [http://192.168.1.106:9980](http://192.168.1.106:9980) para ver se funciona.
...
Acesse [http://192.168.1.106:8005/](http://192.168.1.106:8005/) e verá que aparecerá uma página, significa que está rodando.

```CMD  


Parece que esse aqui é para o Next Cloud...
...
__Organização dos Arquivos dos Usuários
Localização: /var/www/html/data/"nomeDoUsuario"/files
...
Teste: Retirar um arquivo e fazer o App reconhecer.
Para atualizar o BD:
docker exec --user www-data 10ef4302b032 php occ files:scan --all

Para ver qual pasta precisamos colocar o arquivo:
docker exec -it 10ef4302b032 bash
/var/www/html/data/tcnct/files/Documents
docker cp "Nextcloud intro.mp4" 10ef4302b032:/var/www/html/data/tcnct/files/Documents

Verificar se o arquivo subiu
docker exec -it 10ef4302b032 bash
cd /var/www/html/data/tcnct/files/Documents
ls

ok. Pelo WinSCP não aparecec, nem pela web.
exit
docker exec --user www-data 10ef4302b032 php occ files:scan --all

Funcionou...
Agora falta testar com o aplicativo da PlayStore e com a velocidade.
...
Instalar o [onlyoffice](#onlyoffice_documentserver)

[Criar a rede](Producao.md#Rede_666)
...
__Rodar no modo simples____________________________
docker run -d --name filebrowser -p 80:8080 hurlenko/filebrowser

___________________________________________________
__Rodar no modo volume, com persistência de dados__
...
__Copiar os dados__________________________________

Copiar do container para a pasta.
docker cp id_do_container:/data data
docker cp 824ef00007a6:/data data

Copiar da pasta para o container.
Estando num nível acima da pasta... Não é necessário discriminar a pasta data, senão é criada outra pasta data dentro da data.
ls
docker cp data 824ef00007a6:/

__Acessar os arquivos no container_________________
___________________________________________________
docker exec -it 824ef00007a6 sh
cd data
ls

__Acessar os arquivos no volume____________________
___________________________________________________
sudo su
cd /var/lib/docker/volumes/filebrowser/_data
ls
